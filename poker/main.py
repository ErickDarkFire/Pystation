...
def draw_card(surface, card: Card, x: int, y: int, fonts: dict, face_up=True):
    ...
    if not face_up:
        draw_rounded_rect(
            surface, C_CARD_BACK, rect, radius=10, border=2, border_color=C_GOLD
        )
    ...
    draw_rounded_rect(
        surface, C_WHITE, rect, radius=10, border=2, border_color=(200, 200, 200)
    )
    ...
    surface.blit(
        r2, (x + CARD_W - r2.get_width() - 5, y + CARD_H - r2.get_height() - 4)
    )
    surface.blit(
        s2,
        (
            x + CARD_W - s2.get_width() - 5,
...
class Button:
    ...
    def draw(self, surface):
        ...
        lbl = self.font.render(
            self.label, True, C_WHITE if self.enabled else (110, 110, 110)
        )
...
class CasinoPokerApp:
    def __init__(self):
        ...
        self.btn_next = Button(
            (bx(3), BUTTON_Y, BTN_W, BTN_H),
            "NEW ROUND",
            C_BTN_BLUE,
            C_BTN_BLUE_H,
            self.fonts["btn"],
        )
        self.btn_recharge = Button(
            (bx(4), BUTTON_Y, BTN_W, BTN_H),
            "RECHARGE ($500)",
            C_BTN_GOLD,
            C_BTN_GOLD_H,
            self.fonts["btn"],
        ...
        self.btn_minus = Button(
            (minus_x, NUDGE_Y, NUDGE_SZ, NUDGE_SZ),
            "−",
            C_GOLD_DIM,
            C_GOLD,
            self.fonts["btn"],
        ...
        self.btn_plus = Button(
            (plus_x, NUDGE_Y, NUDGE_SZ, NUDGE_SZ),
            "+",
            C_GOLD_DIM,
            C_GOLD,
            self.fonts["btn"],
    ...
    def _update_buttons(self, mouse_pos):
        ...
        self.btn_bet.enabled = (
            phase == GamePhase.FLOP and self.game.chips >= self.game.ante
        )
    ...
    def _draw_background(self):
        ...
        for i in range(10):
            ...
            pygame.draw.rect(
                s,
                (0, 0, 0, 14),
                pygame.Rect(i * 4, i * 4, SCREEN_W - i * 8, SCREEN_H - i * 8),
                width=10,
                border_radius=22,
            )
        ...
        pygame.draw.line(
            self.screen, C_GOLD_DIM, (28, TITLE_H), (SCREEN_W - 28, TITLE_H), 1
        )
        pygame.draw.line(
            self.screen, C_GOLD_DIM, (28, DIVIDER_Y), (SCREEN_W - 28, DIVIDER_Y), 1
        )
    ...
    def _draw_info_bar(self):
        ...
        chip_txt = self.fonts["ui_bold"].render(
            f"Fichas:  ${self.game.chips}", True, C_CHIP
        )
        ...
        if self.game.phase == GamePhase.WAITING_FOR_BET:
            ...
            self.screen.blit(
                ...
                (
                    self._ante_lbl_x,
                    self._ante_val_rect.y
                    + (self._ante_val_rect.h - lbl.get_height()) // 2,
                ),
            )
    ...
    def _draw_community(self):
        ...
        for i in range(5):
            ...
            if i < len(revealed):
                draw_card(
                    self.screen, revealed[i], px, COMMUNITY_Y, self.fonts, face_up=True
                )
            else:
    ...
    def _draw_message(self):
        ...
        self.screen.blit(
            shadow, shadow.get_rect(centerx=SCREEN_W // 2 + 2, top=MESSAGE_Y + 1)
        )
    ...
    def _handle_events(self) -> bool:
        ...
        for event in pygame.event.get():
            ...
            if (
                ...
                and self.game.phase == GamePhase.WAITING_FOR_BET
            ):
            ...
            if self.btn_plus.clicked(event):
                try:
                    self.ante_input = str(
                        min(self.game.chips, int(self.ante_input) + 5)
                    )
                except ValueError:
                    self.ante_input = "5"
            ...
            if self.btn_deal.clicked(event):
                ...
                if self.game.place_ante(amt):
                    ...
                    self.message = (
                        "¡Flop revelado!  Apuesta para ver el turn y river, o retírate."
                    )
                    self.msg_color = C_OFF_WHITE
                else:
                    self.message = (
                        "Ante inválido — debe ser al menos 5 y no superar tus fichas."
                    )
