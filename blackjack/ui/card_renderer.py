...
def draw_suit_shape(surface: pygame.Surface, suit: str, x: float, y: float, scale: float = 1.0, color: tuple = BLACK) -> None:
    """Draws the suit shape perfectly centered at (x, y)."""
    s = 10 * scale
    if suit == '♦':
...
def draw_card(surface: pygame.Surface, card: Card) -> None:
    ...
    pygame.draw.rect(shadow_surf, (0, 0, 0, 100), (0, 0, card.width, card.height), border_radius=8)
    ...
    if card.face_up:
        ...
        rank_txt = FONT_CARD.render(card.rank, True, color)
        surface.blit(rank_txt, (card.x + 8, card.y + 5))
        draw_suit_shape(surface, card.suit, card.x + 20, card.y + 55, 0.8, color)
        draw_suit_shape(surface, card.suit, card.x + card.width/2, card.y + card.height/2, 3.0, color)
    else:
        # Card back pattern
        pygame.draw.rect(surface, (150, 20, 20), rect, border_radius=8)
        pygame.draw.rect(surface, WHITE, (card.x+5, card.y+5, card.width-10, card.height-10), 2, border_radius=6)
