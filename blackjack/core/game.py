...
class Game:
    ...
    def init_ui(self):
        # Botones de juego usando st.WIDTH y st.HEIGHT
        self.btn_deal = Button(
            "DEAL", st.WIDTH // 2 - 60, st.HEIGHT - 120, 120, 50, "deal"
        )
        self.btn_hit = Button(
            "HIT", st.WIDTH // 2 - 200, st.HEIGHT - 120, 120, 50, "hit"
        )
        self.btn_stand = Button(
            "STAND", st.WIDTH // 2 - 60, st.HEIGHT - 120, 120, 50, "stand"
        )
        self.btn_double = Button(
            "DOUBLE", st.WIDTH // 2 + 80, st.HEIGHT - 120, 120, 50, "double"
        )
        self.btn_reset = Button(
            "NEW HAND", st.WIDTH // 2 - 75, st.HEIGHT - 120, 150, 50, "reset"
        )

        # Botones de utilidad y simulación
        self.btn_sim = Button("SIM 10K", 20, st.HEIGHT - 120, 150, 50, "simulate")
        self.btn_add = Button("+ $1000", st.WIDTH - 120, st.HEIGHT - 60, 100, 40, "add")
        self.btn_toggle_coach = Button(
            "COACH: ON",
            st.WIDTH - 180,
            st.HEIGHT - 110,
            160,
        ...
        # Fichas
        self.chip_rects = [
            (pygame.Rect(50 + i * 90, st.HEIGHT - 220, 80, 80), v)
    ...
    def run_simulation(self):
        ...
        for _ in range(total_hands):
            ...
            if calculate_score(p_hand) <= 21:
                while calculate_score(d_hand) < 17:
                    d_hand.append(sim_shoe.draw_card())
                f_p, f_d = calculate_score(p_hand), calculate_score(d_hand)
                m = 2 if advice == "DOUBLE" else 1
                if f_d > 21 or f_p > f_d:
                    wins += 1
                    total_profit += fixed_bet * m
                elif f_p < f_d:
                    losses += 1
                    total_profit -= fixed_bet * m
                else:
                ...
                if e.type == pygame.MOUSEBUTTONDOWN and self.state == "BETTING":
                    for r, v in self.chip_rects:
                        if r.collidepoint(e.pos) and self.player.money >= v:
                            self.player.money -= v
                            self.player.current_bet += v
            self.update(events)
            self.draw()
            self.clock.tick(st.FPS)  # <-- Usando st.FPS

    def update(self, events):
        if self.btn_toggle_coach.update(events) == "toggle_coach":
            self.coach_enabled = not self.coach_enabled
            self.btn_toggle_coach.text = (
                f"COACH: {'ON' if self.coach_enabled else 'OFF'}"
            )

        if self.state == "BETTING":
            if self.btn_sim.update(events) == "simulate":
                self.run_simulation()
            if self.btn_add.update(events) == "add":
                self.player.money += 1000
            if self.btn_deal.update(events) == "deal" and self.player.current_bet > 0:
                self.state = "PLAYING"
                for i in range(2):
                    p_c = self.shoe.draw_card()
                    self.player.hand.append(p_c)
                    self.update_count(p_c)
                    d_c = self.shoe.draw_card()
                    self.dealer.hand.append(d_c)
                    if i == 0:
                        self.update_count(d_c)
            ...
            if self.btn_hit.update(events) == "hit":
                c = self.shoe.draw_card()
                self.player.hand.append(c)
                self.update_count(c)
                organize_hands(self.player.hand, self.dealer.hand)
                if calculate_score(self.player.hand) > 21:
                    self.handle_result("BUST")
            if self.btn_stand.update(events) == "stand":
                self.state = "DEALER_ANIM"
            if (
                self.btn_double.update(events) == "double"
                and self.player.money >= self.player.current_bet
            ):
            ...
            if calculate_score(self.dealer.hand) < 17:
                ...
                s, ps = calculate_score(self.dealer.hand), calculate_score(
                    self.player.hand
                )
                if s > 21 or ps > s:
                    self.handle_result("WIN")
                elif ps < s:
                    self.handle_result("LOSE")
                else:
                    self.handle_result("PUSH")
        ...
        for c in self.player.hand + self.dealer.hand:
            c.update()

    def update_count(self, c):
        if c.rank in ["10", "J", "Q", "K", "A"]:
            self.running_count -= 1
        elif c.rank in ["2", "3", "4", "5", "6"]:
            self.running_count += 1
    ...
    def handle_result(self, r):
        ...
        if r == "LOSE" or r == "BUST":
            self.msg_main, self.msg_sub, self.msg_color = (
                "DEALER WINS",
                f"- ${b}",
                st.LOSE_RED,  # <-- Usando st.LOSE_RED
            )
        elif r == "WIN":
            self.player.money += b * 2
            self.msg_main, self.msg_sub, self.msg_color = (
                "YOU WIN!",
                f"+ ${b}",
                st.WIN_GREEN,  # <-- Usando st.WIN_GREEN
            )
        elif r == "PUSH":
            self.player.money += b
            self.msg_main, self.msg_sub, self.msg_color = (
    ...
    def draw(self):
        ...
        if self.state == "PLAYING" and self.coach_enabled:
            draw_coach(self.screen, self.player.hand, self.dealer.hand)
        ...
        if self.state == "BETTING":
            ...
            draw_chips(self.screen, self.chip_rects)
        elif self.state == "PLAYING":
            ...
            draw_simulation_results(self.screen, self.sim_stats)
            self.btn_reset.draw(self.screen)
        pygame.display.flip()
