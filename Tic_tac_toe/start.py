...
centers_list = [
    [
        (col1_centro, fila1_centro),
        (col2_centro, fila1_centro),
        (col3_centro, fila1_centro),
    ],
    [
        (col1_centro, fila2_centro),
        (col2_centro, fila2_centro),
        (col3_centro, fila2_centro),
    ],
    [
        (col1_centro, fila3_centro),
        (col2_centro, fila3_centro),
        (col3_centro, fila3_centro),
    ],
]

# Puntos donde deben de iniciar las lineas para formar una X
# Linea 1, empieza en -x -y y termina en +x +y
(col1_centro - fig_size, fila1_centro - fig_size), (
    col1_centro + fig_size,
    fila1_centro + fig_size,
)
...
p1_text = Titulos.render("Jugador 1:", 0, (0, 0, 0))
p2_text = Titulos.render("Jugador 2:", 0, (0, 0, 0))
p1_wins_text = fuente.render("Victorias: " + str(p1_wins), 0, (0, 0, 0))
p2_wins_text = fuente.render("Victorias: " + str(p2_wins), 0, (0, 0, 0))
total_games_text = Titulos.render("Partidas: " + str(p1_wins + p2_wins), 0, (0, 0, 0))
p1_won_text = Anuncio.render("Gano el jugador 1!", 0, (255, 0, 0))
p2_won_text = Anuncio.render("Gano el jugador 2!", 0, (0, 0, 255))
...
def Check():
    ...
    if (
        ...
        and tablero[0][1] == tablero[0][2]
        and tablero[0][2] != ""
    ):
        Ganador = tablero[0][0]
    elif (
        tablero[0][0] == tablero[1][0]
        and tablero[1][0] == tablero[2][0]
        and tablero[2][0] != ""
    ):
        Ganador = tablero[0][0]
    elif (
        tablero[0][0] == tablero[1][1]
        and tablero[1][1] == tablero[2][2]
        and tablero[2][2] != ""
    ):
        Ganador = tablero[0][0]
    elif (
        tablero[0][1] == tablero[1][1]
        and tablero[1][1] == tablero[2][1]
        and tablero[2][1] != ""
    ):
        Ganador = tablero[0][1]
    elif (
        tablero[0][2] == tablero[1][2]
        and tablero[1][2] == tablero[2][2]
        and tablero[2][2] != ""
    ):
        Ganador = tablero[0][2]
    elif (
        tablero[2][0] == tablero[1][1]
        and tablero[1][1] == tablero[0][2]
        and tablero[0][2] != ""
    ):
        Ganador = tablero[2][0]
    elif (
        tablero[1][0] == tablero[1][1]
        and tablero[1][1] == tablero[1][2]
        and tablero[1][2] != ""
    ):
        Ganador = tablero[1][0]
    elif (
        tablero[2][0] == tablero[2][1]
        and tablero[2][1] == tablero[2][2]
        and tablero[2][2] != ""
    ):
        Ganador = tablero[2][0]
...
def Draw_game():
    for i in range(3):
        for j in range(3):
            if tablero[i][j] == "X":
                pygame.draw.line(
                    screen, RED, p1_list[i][j], p2_list[i][j], fig_size // 3
                )
                pygame.draw.line(
                    screen, RED, p3_list[i][j], p4_list[i][j], fig_size // 3
                )

            elif tablero[i][j] == "O":
                pygame.draw.circle(
                    screen, BLUE, centers_list[i][j], fig_size, circle_width
                )
    ...
    if tablero[current_x][current_y] == "":
        if turno == 0:
            pygame.draw.line(
                screen,
                RED,
                p1_list[current_x][current_y],
                p2_list[current_x][current_y],
                fig_size // 3,
            )
            pygame.draw.line(
                screen,
                RED,
                p3_list[current_x][current_y],
                p4_list[current_x][current_y],
                fig_size // 3,
            )
        else:
            pygame.draw.circle(
                screen, BLUE, centers_list[current_x][current_y], fig_size, circle_width
            )
...
def Reinicio():
    ...
    return 0, "X", False, False, 0, Escoger_color(), False
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                ...
                # Reinicio del juego
                turno, fig, clic, win, pandeo_cont, color_selec, victoria_procesada = (
                    Reinicio()
                )
    ...
    screen.blit(p1_text, (ancho_marco - 190, 0))
    screen.blit(p1_wins_text, (ancho_marco - 190, 50))
    screen.blit(p2_text, (ancho_marco - 190, alto // 2))
    ...
    if win and not victoria_procesada:
        ...
        p1_wins_text = fuente.render("Victorias: " + str(p1_wins), 0, (0, 0, 0))
        p2_wins_text = fuente.render("Victorias: " + str(p2_wins), 0, (0, 0, 0))
        total_games_text = Titulos.render(
            "Partidas: " + str(p1_wins + p2_wins), 0, (0, 0, 0)
        )
    ...
    if win:
        if turno == 1:
            screen.blit(p1_won_text, (ancho // 4, (alto // 2) - 40))
        elif turno == 0:
            screen.blit(p2_won_text, (ancho // 4, (alto // 2) - 40))
