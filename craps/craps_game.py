...
class CrapsGame:
    ...
    def procesar_tiro(self):
        ...
        if self.punto is None:
            if resultado in (7, 11):
                ...
                self.ganadas += 1; self.saldo += self.apuesta
            elif resultado in (2, 3, 12):
                self.mensaje = f"Craps {resultado}. Perdiste ${self.apuesta}"
                self.perdidas += 1; self.saldo -= self.apuesta
            else:
    ...
    def dibujar(self):
        ...
        txt_st = self.font.render(f"G: {self.ganadas} | P: {self.perdidas}", True, (255, 255, 255))
        self.screen.blit(txt_st, (20, 20))

        if self.mostrar_dados:
            self.screen.blit(self.dice_images[self.dados_actuales[0]], (self.WIDTH//2 - 110, 200))
            self.screen.blit(self.dice_images[self.dados_actuales[1]], (self.WIDTH//2 + 10, 200))
